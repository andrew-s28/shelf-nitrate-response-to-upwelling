...
nitrate.attrs["created_on"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
# add metadata that includes deployment numbers in dataset and creation script and time
deployments = np.unique(midshelf_nitrate.deployment.values)[
...
midshelf_nitrate.attrs["created_on"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
